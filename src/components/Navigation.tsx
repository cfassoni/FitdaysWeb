...
export function Navigation() {
  ...
  const navItems = [
    ...
    { href: "/body-composition", label: t("nav.bodyComposition"), icon: Activity },
    { href: "/nutrition", label: t("nav.nutrition"), icon: Utensils },
    { href: "/activities", label: t("nav.activities"), icon: Dumbbell },
      ...
      <aside className="w-64 bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 flex flex-col justify-between p-4 min-h-screen">
        ...
        <div className="space-y-3">
          <div 
            ...
            className="p-3 rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800 flex items-center gap-3 cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"
          >
            <div className="p-2 rounded-lg bg-white dark:bg-slate-700 shadow-sm text-slate-700 dark:text-slate-300">
              <FileSpreadsheet className="w-4 h-4" />
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-xs font-semibold text-slate-900 dark:text-slate-100 truncate">{t("nav.importData")}</p>
              <p className="text-[11px] text-slate-500 truncate">{t("nav.importDesc")}</p>
            </div>
          </div>
          ...
          <form action={logout}>
            <button className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium text-slate-600 hover:text-rose-600 hover:bg-rose-50 dark:text-slate-400 dark:hover:text-rose-400 dark:hover:bg-rose-950/50 transition-colors">
