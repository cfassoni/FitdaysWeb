...
export default function SharedReports({ onLinksUpdated }: SharedReportsProps) {
  ...
  const fetchLinks = async () => {
    try {
      ...
      setLinks(data);
      if (onLinksUpdated) {
        const count = data.filter((l) => !l.expires_at || new Date(l.expires_at) > new Date()).length;
        onLinksUpdated(count);
      }
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : "";
      setError(msg || t("sharing.errorLoad"));
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSaveEditSubmit = async () => {
    ...
    try {
      ...
      if (onLinksUpdated) {
        const count = nextLinks.filter((l) => !l.expires_at || new Date(l.expires_at) > new Date()).length;
        onLinksUpdated(count);
      }
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : "";
      setEditError(msg || t("sharing.errorUpdate"));
    } finally {
  ...
  const handleRevokeConfirm = async () => {
    ...
    try {
      ...
      const msg = err instanceof Error ? err.message : "";
      setError(msg || t("sharing.errorRevoke"));
    } finally {
      setIsRevoking(false);
    }
  };
    ...
    <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-background">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">{t("sharing.subtitle")}</p>
        </div>
                  ...
                  return (
                    <tr key={link.id} className={`hover:bg-muted/15 transition-colors ${!active ? "opacity-55" : ""}`}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-muted-foreground">
                        {t("sharing.recordCount", { count: link.entry_count })}
                        {link.include_attachments && t("sharing.withFiles")}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-muted-foreground">
          ...
          <div className="relative w-full max-w-md bg-card border border-border rounded-xl shadow-2xl p-6 z-10 text-foreground">
            <div className="flex items-start gap-3 mb-4">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-1">{t("sharing.revokeConfirm")}</p>
              </div>
            </div>
