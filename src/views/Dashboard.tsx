...
export default function Dashboard({ onNavigateToImport }: DashboardProps) {
  ...
  useEffect(() => {
    const savedFat = localStorage.getItem("recomp_pro_body_fat_unit");
    if (savedFat === "pct" || savedFat === "kg") setBodyFatUnit(savedFat);
    const savedMuscle = localStorage.getItem("recomp_pro_muscle_unit");
    if (savedMuscle === "kg" || savedMuscle === "pct") setMuscleUnit(savedMuscle);
  }, []);

  const handleBodyFatUnitChange = (unit: "pct" | "kg") => {
    setBodyFatUnit(unit);
    localStorage.setItem("recomp_pro_body_fat_unit", unit);
  };

  const handleMuscleUnitChange = (unit: "kg" | "pct") => {
    setMuscleUnit(unit);
    localStorage.setItem("recomp_pro_muscle_unit", unit);
  };
  ...
  useEffect(() => {
    const fetchSummary = async () => {
      try {
        ...
        setError(msg || t("dashboard.errorLoad"));
      } finally {
        setIsLoading(false);
      }
    };
              ...
              <LineChart data={formattedChartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                ...
                <Tooltip
                  formatter={(value: unknown, name?: unknown) => {
                    const valNum = typeof value === "number" ? value.toFixed(1) : String(value);
