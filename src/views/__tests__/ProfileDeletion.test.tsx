...
function getTranslation(obj: unknown, path: string): string {
...
describe("Profile Data Deletion and Account Removal", () => {
  ...
  it("opens Option A modal and successfully calls deleteUserData", async () => {
    vi.mocked(api.deleteUserData).mockResolvedValueOnce({
      message: "Deleted",
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByText("All your health data and shared links have been permanently deleted.")
      ).toBeInTheDocument();
    });
  });

  it("handles error in Option A modal when password is wrong", async () => {
    vi.mocked(api.deleteUserData).mockRejectedValueOnce(new Error("Incorrect password"));
  ...
  it("opens Option B modal and successfully deletes account and dispatches session expired event", async () => {
    vi.mocked(api.deleteAccount).mockResolvedValueOnce({ message: "Account deleted" });
