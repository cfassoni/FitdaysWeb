...
export default function History({ onLinksUpdated }: { onLinksUpdated?: (count: number) => void } = {}) {
    ...
    try {
      ...
      if (onLinksUpdated) {
        try {
          ...
          const count = links.filter((l) => !l.expires_at || new Date(l.expires_at) > new Date()).length;
          onLinksUpdated(count);
        } catch (err) {
    ...
    try {
      ...
      if (failCount === 0) {
        ...
        setDeleteFeedback({
          ...
          message: `${t("history.deletePrompt.successMultiple", {
            deletedCount: successCount,
          })}, ${t("history.deletePrompt.failedCount", { failedCount: failCount })}.\n${failReasons}`,
        });
      }
      ...
      setDeleteFeedback({
        success: false,
        title: t("history.deletePrompt.failedTitle"),
        message: msg || t("history.deletePrompt.errorGeneric"),
      });
      setIsDeleteConfirmOpen(false);
      setIdsToDelete([]);
    } finally {
  ...
  const handleUploadSubmit = async () => {
    ...
    try {
      ...
      const msg = err instanceof Error ? err.message : "";
      setUploadError(msg || t("history.reportUploadError"));
      setUploadProgress(null);
    }
  };
  ...
  const handleDeleteReport = async (recordId: number) => {
    ...
    try {
      ...
      setDeleteFeedback({
        ...
        message: msg || t("history.report.deleteFailed"),
      });
    }
  };
  ...
  useEffect(() => {
    const fetchRecords = async () => {
      try {
        ...
        const msg = err instanceof Error ? err.message : "";
        setError(msg || t("history.errorLoad"));
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const getSortedRecords = () => {
    return [...records]
      ...
      .sort((a, b) => {
        ...
        if (sortField === "date") {
          valA = new Date(a.date).getTime() as never;
          valB = new Date(b.date).getTime() as never;
        }
  ...
  const formatSegmentValue = (value: number | null | undefined, unit = "") => {
  ...
  return (
    <div className="flex-1 p-6 overflow-y-auto max-w-7xl mx-auto w-full space-y-6 animate-in fade-in duration-300">
      ...
      {deleteFeedback && (
        <div
          className={`p-4 rounded-xl border flex items-start gap-3 animate-in fade-in slide-in-from-top duration-300 ${
        ...
        >
          ...
          <div className="flex-1">
            <p className="text-sm font-semibold">
              {deleteFeedback.title ||
                (deleteFeedback.success
                  ? t("history.deletePrompt.successTitle")
                  : t("history.deletePrompt.failedTitle"))}
            </p>
      ...
      <div className="flex flex-col sm:flex-row gap-4 items-center justify-between">
        ...
        <div className="text-xs text-muted-foreground font-medium self-end sm:self-center">
          {t("history.showingRecords", { count: sortedRecords.length, total: records.length }) ||
      ...
      <div className="bg-card border border-border rounded-xl shadow-xs overflow-hidden">
        {sortedRecords.length === 0 ? (
          <div className="p-12 text-center text-muted-foreground text-sm">{t("history.noMatch")}</div>
        ) : (
        ...
        <div className="fixed inset-0 z-50 flex justify-end">
          ...
          <div className="relative w-full max-w-2xl bg-card border-l border-border h-full flex flex-col shadow-2xl animate-in slide-in-from-right duration-300 z-10">
            ...
            <div className="flex items-center justify-between px-6 py-5 border-b border-border">
              ...
              <button
                onClick={() => setSelectedRecord(null)}
                className="p-2 text-muted-foreground hover:text-foreground rounded-lg hover:bg-muted border border-border cursor-pointer"
              >
                <X className="h-5 w-5" />
              </button>
            </div>
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                          ...
                          <div className="bg-muted/30 border border-border p-2 rounded-lg text-center">
                            <span className="block text-[10px] text-muted-foreground uppercase font-semibold">
                              {t("history.drawer.fatMass")}
                            </span>
                            <span className="text-xs font-bold text-rose-500">
                              {formatSegmentValue(fatMass, " kg")}
                            </span>
                            <span className="block text-[9px] text-muted-foreground mt-0.5">
                              {formatSegmentValue(fatPct, "%")} ({fatLevel || t("history.drawer.normal")})
                            </span>
                          </div>
                          ...
                          <div className="bg-muted/30 border border-border p-2 rounded-lg text-center">
                            <span className="block text-[10px] text-muted-foreground uppercase font-semibold">
                              {t("history.drawer.muscleMass")}
                            </span>
                            <span className="text-xs font-bold text-emerald-600 dark:text-emerald-400">
                              {formatSegmentValue(musMass, " kg")}
                            </span>
                            <span className="block text-[9px] text-muted-foreground mt-0.5">
                              {formatSegmentValue(musPct, "%")} ({musLevel || t("history.drawer.normal")})
                            </span>
                          </div>
          ...
          <div className="relative w-full max-w-md bg-card border border-border rounded-xl shadow-2xl p-6 space-y-4 animate-in fade-in zoom-in-95 duration-250 z-10">
            ...
            >
              ...
              <div className="space-y-1">
                <p className="text-sm font-semibold text-foreground">{t("history.report.dragDrop")}</p>
                <p className="text-xs text-muted-foreground">{t("history.report.browseDevice")}</p>
              </div>
      ...
      {isShareModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
          {/* Backdrop */}
          <div className="fixed inset-0 bg-black/60 backdrop-blur-xs transition-opacity" onClick={handleCloseShareModal} />
                ...
                <div>
                  ...
                  <div className="grid grid-cols-3 sm:grid-cols-5 gap-2">
                    {(["never", "1d", "7d", "30d", "custom"] as const).map((option) => (
                      ...
                      >
                        {t(`sharing.expires${option.charAt(0).toUpperCase() + option.slice(1)}` as never) || option}
                      </button>
                    ))}
                  </div>
