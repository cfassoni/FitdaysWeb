...
CREATE UNIQUE INDEX IF NOT EXISTS `_user_date_uc` ON `fitdays_records` (`user_id`,`date`);--> statement-breakpoint
CREATE TABLE IF NOT EXISTS `fitdays_reports` (
	...
	FOREIGN KEY (`record_id`) REFERENCES `fitdays_records`(`id`) ON UPDATE no action ON DELETE cascade
);
--> statement-breakpoint
CREATE UNIQUE INDEX IF NOT EXISTS `fitdays_reports_record_id_unique` ON `fitdays_reports` (`record_id`);--> statement-breakpoint
...
--> statement-breakpoint
CREATE UNIQUE INDEX IF NOT EXISTS `shared_links_token_unique` ON `shared_links` (`token`);--> statement-breakpoint
