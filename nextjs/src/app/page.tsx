...
export default function App() {
  ...
  const isVerifyEmailRoute =
    ...
    (window.location.pathname === "/verify-email" || new URLSearchParams(window.location.search).has("code"));
  ...
  const [currentView, setCurrentView] = useState<"dashboard" | "history" | "import" | "profile" | "shared_links">(
    "dashboard"
  );
  ...
  const isSharedPath = typeof window !== "undefined" && window.location.pathname.startsWith("/shared/");
  ...
  if (!isAuthenticated) {
    if (authView === "verify") {
      return <VerifyEmail initialEmail={verifyEmailTarget} onGoToLogin={() => setAuthView("login")} />;
    }
    ...
    if (authView === "register") {
      return <Register onRegisterSuccess={handleLoginSuccess} onGoToLogin={() => setAuthView("login")} />;
    }
    ...
    <div className="flex flex-col h-screen w-screen overflow-hidden bg-background text-foreground">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <main className="flex-1 flex flex-col h-full overflow-hidden">
          {currentView === "dashboard" && <Dashboard onNavigateToImport={() => setCurrentView("import")} />}
          {currentView === "history" && <History onLinksUpdated={setSharedLinksCount} />}
          {currentView === "import" && <Import onImportSuccess={() => setCurrentView("dashboard")} />}
          {currentView === "profile" && (
            <Profile user={user} onProfileUpdated={(updatedUser) => setUser(updatedUser)} />
          )}
          {currentView === "shared_links" && <SharedReports onLinksUpdated={setSharedLinksCount} />}
        </main>
      </div>
    </div>
  );
