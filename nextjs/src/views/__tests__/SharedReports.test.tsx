...
describe("SharedReports View - i18n & Interactions", () => {
  ...
  it("renders shared links table with localized tooltips and content", async () => {
    ...
    expect(
      screen.getByText("Manage shared guest access links for doctors, nutritionists, or coaches")
    ).toBeInTheDocument();
  ...
  it("opens the Revoke confirmation dialog and verifies localized headers and buttons", async () => {
    vi.mocked(api.getSharedLinks).mockResolvedValue(mockLinks);
    vi.mocked(api.deleteSharedLink).mockResolvedValue(undefined as never);
    ...
    expect(
      screen.getByText(
        "Are you sure you want to revoke this shared link? It will become inaccessible immediately."
      )
    ).toBeInTheDocument();
